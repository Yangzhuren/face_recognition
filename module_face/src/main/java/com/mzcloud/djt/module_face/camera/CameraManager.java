...
//      }
    ...
    public void requestAutoFocus(Handler handler, int message) {
        if (camera != null && previewing) {
            ...
            float[] f = new float[3];
            camera.getParameters().getFocusDistances(f);
            //if 20cm<FocusDistances<30cm , do not autofocus
            //if(f[1]>0.3 || f[1]<0.2)
...
//                topOffset = (screenResolution.y - FRAME_HEIGHT) / 2;
            topOffset = (int) (0.5f + 150 * Resources.getSystem().getDisplayMetrics().density);
        }
...
//      rect.bottom = rect.bottom * cameraResolution.y / screenResolution.y;
    ...
    public void flashHandler(ImageView iv, TextView tv) {
            ...
            if (PackageManager.FEATURE_CAMERA_FLASH.equals(f.name)) {
                if (camera != null) {
                    ...
                    if (Parameters.FLASH_MODE_OFF.equals(parameters.getFlashMode())) {
                        turnOn(parameters);
                        iv.setBackgroundResource(R.drawable.open);
                        tv.setText("关闭手电筒");
                    } else if (Parameters.FLASH_MODE_TORCH.equals(parameters.getFlashMode())) {
                        iv.setBackgroundResource(R.drawable.close_sdt);
                        turnOff(parameters);
                        tv.setText("打开手电筒");
                    }
                }
